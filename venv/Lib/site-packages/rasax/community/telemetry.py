...
from typing import Any, Dict, Optional, Text, TYPE_CHECKING, Union, List, NoReturn
...
async def _get_project_status_event(
    ...
    nlu_inbox_messages_with_tags = set()
    for tag in tags:
        conversations_with_tags.update(tag["conversations"])
        nlu_inbox_messages_with_tags.update(tag["message_logs"])
    ...
    return {
        ...
        "num_nlu_inbox_messages_with_tags": len(nlu_inbox_messages_with_tags),
        "num_e2e_tests": len(
            test_stories.result
...
def _disable_telemetry() -> NoReturn:  # type: ignore[misc]
